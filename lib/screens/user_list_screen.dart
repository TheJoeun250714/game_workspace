...
class UserListScreen extends StatefulWidget {
  ...
  const UserListScreen({super.key, required this.gameId, this.gameItem});
...
class _UserListScreenState extends State<UserListScreen> {
  ...
  @override
  void initState() {
    super.initState(); // 기존에 플러터 개발자가 만들어놓은 초기 세팅포함하여
  ...
  Future<void> loadUsers() async {
    ...
    try {
      ...
      setState(() {
        ...
        //  users  변수에서 선언한 모델 타입이 일치하는지 반드시 확인
    ...
    return Scaffold(
        ...
        body: isLoading
            ...
            : users.isEmpty // 로딩상태는 아니지만 로딩 결과 존재하는 유저가 없을 때
                ...
                : ListView.builder(
                    // ListView 목록을 DB에서 가져와서 만들 때는
                    itemCount:
                        users.length, // 반드시 DB에서 조회되는 총 데이터의 개수를 ui에 알려주기
                    itemBuilder: (context, index) {
                      // 데이터가 존재하면 생성하겠다. (현재scaffold 위치에서, 0번부터 순서대로 배치)
