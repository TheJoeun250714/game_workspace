...
class UserProvider extends ChangeNotifier{
  ...
  // 새로운 유저가 추가된다면 추가된 유저 정보 DB에서 다시 업데이트하여 상태관리
  Future<void> loadAllUsers() async {
    _isLoading = true;
    notifyListeners();
    // static Future<List<User>> getAllUsers() async {
    // 만약에 getAllUsers() 가 static 모두가 사용할 수 있게 static 형태로 선언되어 있다면
    // 인스턴스형태로 호출하여 사용 불가
  ...
  // 로그인한다면 현재 유저정보를 가져와서 적용하는 상태관리
  ...
  // 프로필 이미지만 현재 유저가 변경한다면 변경된 데이터에 관련되어 다시 적용하는 상태관리
  // 위 loadUser 만으로도 프로필 변경 업데이트된 유저 정보를 호출 할 수 있지만
  // DB에서 유저 정보에 대한 모~든 정보를 다시 가져와야 하므로
  // 메모리 소모 / 트래픽 소모를 줄이기 위해서 다른 데이터는 그대로 사용하되 특정 데이터만
  // db에서 호출하여 일부 변경한다 라는 구조
